...
logger.info("--- SYSTEM BOOT: V9.1 OPTIMIZED SMART CHUNKING EDITION (BOT TOKEN VERSION) ---")
...
# --- 4. PARSE CONFIG FROM COMMAND (OPTIMIZED WITH REGEX) ---
...
def validate_config(config: Dict) -> Tuple[bool, str]:
    ...
    try:
        ...
        config['batch_time'] = DEFAULT_BATCH_TIME
...
async def copy_worker(job: Dict, status_msg):
    ...
    try:
        for chunk_start in range(start_id, end_id + 1, chunk_size):
            ...
            chunk_end = min(chunk_start + chunk_size - 1, end_id)  # Adjust to inclusive
            ...
            for msg in messages_batch:
                ...
                for retry_idx in range(max_retries):
                    ...
                    try:
                        ...
                        # Continue retry without incrementing retry_idx
                ...
                else:  # No break, meaning failed after retries
                    stats['failed'] += 1
                ...
                if time.time() - last_update_time > 10:
                    ...
                    text = (
                        f"🐎 **WORKHORSE V9.1: OPTIMIZED SMART CHUNKING (BOT VERSION)**\n"
                        f"{bar_str}\n\n"
                        f"📊 **Logistik Kargo:**\n"
...
async def start_cmd(client, message):
    ...
    try:
        ...
        # --- FITUR ANTI-AMNESIA ---
        status_msg = await message.reply("🚑 **Sedang Menyembuhkan Sesi (Anti-Amnesia)...**")
        try:
            count = 0
            async for dialog in app.get_dialogs(limit=50):
                count += 1
            await status_msg.edit(f"✅ **Sesi Pulih! (Memuat {count} chat)**\n🐎 **Menyiapkan Kuda...**")
        except Exception as e:
            logger.warning(f"Gagal refresh dialog: {e}")
            await status_msg.edit(f"⚠️ **Gagal Refresh Sesi:** {e}\nTetap mencoba lanjut...")
...
async def stop_cmd(client, message):
    if IS_WORKING:
        STOP_EVENT.set()
        await message.reply("🛑 **REM DARURAT DITARIK!**\nMenunggu proses terakhir selesai...")
    else:
        await message.reply("💤 Bot sedang tidur.")

@app.on_message(filters.command("stats") & filters.group)
async def stats_cmd(client, message):
    cpu_val, cpu_txt, ram_val, _ = get_system_status(0)
    status_bot = "🔥 SEDANG LEMBUR" if IS_WORKING else "💤 SEDANG ISTIRAHAT"
    text = (
        f"🐴 **STATUS SERVER (V9.1 OPTIMIZED EMBER EDITION - BOT VERSION)**\n"
        f"──────────────────\n"
        f"🤖 **Status:** {status_bot}\n"
        f"🌡️ **Resource Bot:**\n"
        f"• 🧠 CPU: {cpu_val}% [{cpu_txt}]\n"
...
async def web_handler(request):
    return web.Response(text="Bot Running V9.1 (Bot Version).")
